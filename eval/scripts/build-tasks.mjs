...
try {
  execFileSync('node', [join(ROOT, '..', 'node_modules', 'typescript', 'bin', 'tsc'), '--outDir', COMPILED, '--module', 'NodeNext', '--moduleResolution', 'NodeNext', '--target', 'ES2022', '--esModuleInterop', join(ROOT, 'tasks', 'ts-tasks.ts'), join(ROOT, 'tasks', 'hard-tasks.ts'), join(ROOT, 'tasks', 'trap-tasks.ts'), join(ROOT, 'tasks', 'noselftest-tasks.ts'), join(ROOT, 'tasks', 'complex-tasks.ts'), join(ROOT, 'tasks', 'implicit-tasks.ts'), join(ROOT, 'types.ts')], { stdio: 'pipe' });
} catch (e) {
...
const { implicitTasks } = await import(join(COMPILED, 'tasks', 'implicit-tasks.js'));
const all = [...tsTasks, ...hardTasks, ...trapTasks, ...noselftestTasks, ...complexTasks, ...implicitTasks];
writeFileSync(join(ROOT, 'tasks', 'tasks.json'), JSON.stringify(all, null, 2));
console.log(`Wrote eval/tasks/tasks.json with ${all.length} tasks (${tsTasks.length} easy + ${hardTasks.length} hard + ${trapTasks.length} trap + ${noselftestTasks.length} no-self-test + ${complexTasks.length} complex + ${implicitTasks.length} implicit)`);
